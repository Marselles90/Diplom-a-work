...
class UserManager(BaseUserManager):
    ...
    def create_superuser(self, email, number,  password=None, **extra_fields):
        user = self.create_user(email, number, password=password)
...
class User(AbstractUser):
    ...
    number = models.CharField(max_length=15, verbose_name='Номер телефона', unique=True)
    ...
    REQUIRED_FIELDS = ['first_name', 'last_name', 'number', 'username']
